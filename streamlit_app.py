...
def init_session_state():
    if 'markdown_editor' not in st.session_state: st.session_state['markdown_editor'] = ""
    if 'raw_extracted_content' not in st.session_state: st.session_state['raw_extracted_content'] = ""
    if 'podcast_script' not in st.session_state: st.session_state['podcast_script'] = []
...
def generate_slides_content():
    ...
    if len(raw) < 50: st.error("Content too short."); return
    with st.spinner("🤖 Generating Slides (Gemini)..."):
        res = generate_structured_markdown(raw)
        st.session_state['markdown_editor'] = res
        if "API Error" in res: st.error("Rate Limit or API Error.")
        else: st.success("✅ Slides Generated")

def generate_audio_logic():
    raw = st.session_state.get('raw_extracted_content', '')
    if not raw: st.error("No content."); return
...
def main_app_ui():
    ...
    with col1: file_up = st.file_uploader("Upload File", type=['pdf','docx','txt'])
    if st.button("Import File") and file_up: import_file_content(file_up, None)
    
    with st.form("url_form"):
        url = st.text_input("Or Enter URL")
        if st.form_submit_button("Import URL"): import_from_url(url, None)
        
    # 1.5 Audio
    st.markdown("---")
    st.header("🎧 Audio Overview")
    if len(st.session_state.get('raw_extracted_content', '')) > 0:
        if st.button("Generate Audio Overview", icon="🎙️"): generate_audio_logic()
        if st.session_state['audio_path']:
            st.audio(st.session_state['audio_path'])
            with st.expander("View Script"):
                for l in st.session_state['podcast_script']:
                    st.write(f"**{l.get('speaker')}**: {l.get('text')}")
    else: st.info("Import content first.")
    ...
    st.text_area("Content", value=st.session_state.get('markdown_editor',''), height=400, key="markdown_editor")
